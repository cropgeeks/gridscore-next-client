              ...
              <b-row>
                <b-col cols=6>
                  <b-form-group :description="$t('formDescriptionTraitTimeframeStart')" label-for="trait-timeframe-start">
                    <template #label>
                      <BIconstack><BIconCalendar /><BIconChevronBarLeft :scale="0.6" :shift-v="-2" /></BIconstack> {{ $t('formLabelTraitTimeframeStart') }}
                    </template>
                ...
                <b-col cols=6>
                  <b-form-group :description="$t('formDescriptionTraitTimeframeEnd')" label-for="trait-timeframe-end">
                    <template #label>
                      <BIconstack><BIconCalendar /><BIconChevronBarRight :scale="0.6" :shift-v="-2" /></BIconstack> {{ $t('formLabelTraitTimeframeEnd') }}
                    </template>
        ...
        <draggable v-model="traits" tag="b-list-group" v-if="traits && traits.length > 0" handle=".drag-handle" class="trait-list">
          <b-list-group-item :active="newTrait.id === trait.id" href="#" @click="toggleTrait(index)" class="flex-column align-items-start" v-for="(trait, index) in traits" :key="`trait-list-${trait.id}`">
            ...
            <div v-if="trait.timeframe">
              ...
              <span class="mr-2" v-if="trait.timeframe.start"><BIconstack><BIconCalendar /><BIconChevronBarLeft :scale="0.6" :shift-v="-2" /></BIconstack> {{ trait.timeframe.start }}</span>
              <span class="mr-2" v-if="trait.timeframe.end"><BIconstack><BIconCalendar /><BIconChevronBarRight :scale="0.6" :shift-v="-2" /></BIconstack> {{ trait.timeframe.end }}</span>
            </div>
