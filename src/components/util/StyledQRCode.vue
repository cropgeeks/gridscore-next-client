  ...
  <div v-if="code">
    ...
    <template v-if="showCode">
      ...
      <v-text-field
        ...
        hide-details
      >
        <template #append-inner v-if="canShare">
          <v-icon
            :icon="mdiShare"
            @click="share"
            v-tooltip:top="$t('buttonShare')"
          />
        </template>
      </v-text-field>
    </template>
...
<script lang="ts" setup>
  ...
  const code = computed(() => {
    if (compProps.text) {
      if (compProps.isTrialCode) {
        // @ts-ignore
        const uuidPart = router.resolve({ path: `/trial/import/${compProps.text}` }).path

        if (compProps.baseUrl) {
          return compProps.baseUrl + '#' + uuidPart
        } else {
  ...
  function share () {
    try {
      navigator.share({
        title: 'GridScore trial share code',
        url: code.value,
      })
    } catch {
      // Do nothing here
